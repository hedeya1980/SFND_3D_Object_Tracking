...
int main(int argc, const char *argv[])
{
    ...
    // data location
    string dataPath = "../";
    //string dataPath = "C:/workspace/SFND_3D_Object_Tracking-master/";
    ...
    std::vector<float> TTCLidar;
    std::vector<float> TTCamera;
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    if (bVis)
                    {
                        ...
                        //std::cout<<ttcLidar<<", "<<ttcCamera<<endl;
    ...
    cout << "\n" << "TTC LIDAR" << "\t" << "TTC CAMERA" << endl;
    cout << "--------------------------" << endl;
    for(int i=0;i<TTCamera.size();i++)
    {
        cout<<TTCLidar[i]<< "\t\t" <<TTCamera[i]<<endl;
    }
    std::getchar();
